...
class DashboardFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, app):
        ...
        ctk.CTkLabel(self, text="Próximas Entregas (7 dias)", font=ctk.CTkFont(size=14, weight="bold"),
                     text_color="#c0c0d8").grid(row=4, column=0, columnspan=4, sticky="w", pady=(0, 12))
    ...
    def _row(self, t):
        ...
        ctk.CTkLabel(row, text=t['titulo'], font=ctk.CTkFont(size=13, weight="bold"),
                     text_color="#d0d0e8", anchor="w").grid(row=0, column=1, sticky="w")
        ctk.CTkLabel(row, text=f"{t['disciplina']}  •  {dias}d", font=ctk.CTkFont(size=11),
                     text_color="#666680", anchor="e").grid(row=0, column=2, padx=16)
...
class ListarFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, app):
        ...
        ctk.CTkLabel(hdr, text="Todas as Tarefas", font=ctk.CTkFont("Georgia", 28, "bold"),
                     text_color="#e8d5b0").grid(row=0, column=0, sticky="w")

        btns = ctk.CTkFrame(hdr, fg_color="transparent")
        btns.grid(row=0, column=1, sticky="e")

        ctk.CTkButton(btns, text="↻ Atualizar", height=36, width=110,
                      fg_color="#1e1e2e", hover_color="#23233a", text_color="#a0a0c0",
                      corner_radius=8, command=self.refresh).grid(row=0, column=0, padx=(0, 8))
        ctk.CTkButton(btns, text="＋ Nova", height=36, width=90,
                      fg_color="#3a3a6a", hover_color="#4a4a8a", text_color="#e8d5b0",
                      corner_radius=8, command=lambda: app.show_frame("cadastrar")).grid(row=0, column=1)

        self.filtro = ctk.CTkSegmentedButton(self, values=["Todas", "Pendentes", "Concluídas"],
                                              fg_color="#17171f", selected_color="#3a3a6a",
        ...
        try:
            ...
            if not tarefas:
                ctk.CTkLabel(self.scroll, text="Nenhuma tarefa encontrada.",
                             text_color="#555570", font=ctk.CTkFont(size=13)).grid(padx=20, pady=30)
            else:
                for t in tarefas:
                    self._row(t)
        except Exception as e:
    ...
    def _row(self, t):
        ...
        dot = ctk.CTkLabel(row, text="●", text_color=cor if not t['ok'] else "#444458",
        ...
        ctk.CTkLabel(row, text=titulo_text, font=ctk.CTkFont(size=13, weight="bold"),
                     text_color=titulo_color, anchor="w").grid(row=0, column=1, sticky="w", pady=(12, 2))

        info = f"{t['disciplina'] or '—'}  •  {PRIORIDADE[t['prioridade']]}  •  {t['data'].strftime('%d/%m/%Y')}"
        ctk.CTkLabel(row, text=info, font=ctk.CTkFont(size=11),
                     text_color="#555570", anchor="w").grid(row=1, column=1, sticky="w", pady=(0, 12))

        dias = dias_restantes(t['data'])
        if not t['ok']:
            dias_txt = f"{dias}d" if dias >= 0 else f"Atrasada {abs(dias)}d"
            dias_cor = "#4CAF50" if dias > 3 else ("#FF9800" if dias >= 0 else "#F44336")
            ctk.CTkLabel(row, text=dias_txt, font=ctk.CTkFont(size=12, weight="bold"),
                         text_color=dias_cor).grid(row=0, column=2, rowspan=2, padx=12)

        actions = ctk.CTkFrame(row, fg_color="transparent")
        actions.grid(row=0, column=3, rowspan=2, padx=(0, 12))

        if not t['ok']:
            ctk.CTkButton(actions, text="✓", width=32, height=32,
                          fg_color="#1a2e1a", hover_color="#1f3d1f",
                          text_color="#4CAF50", corner_radius=8,
                          command=lambda tid=t['id']: self.concluir(tid)).grid(row=0, column=0, padx=2)
        ctk.CTkButton(actions, text="✕", width=32, height=32,
                      fg_color="#2e1a1a", hover_color="#3d1f1f",
                      text_color="#F44336", corner_radius=8,
                      command=lambda tid=t['id'], ttl=t['titulo']: self.excluir(tid, ttl)).grid(row=0, column=1, padx=2)
...
class RelatoriosFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, app):
        ...
        ctk.CTkLabel(self, text="Relatórios", font=ctk.CTkFont("Georgia", 28, "bold"),
                     text_color="#e8d5b0").grid(row=0, column=0, sticky="w", pady=(0, 20))
    ...
    def _load_atrasadas(self):
        ...
        try:
            ...
            conn.close()
            if not atr:
                ctk.CTkLabel(self.scroll_atr, text="Nenhuma tarefa atrasada 🎉",
                             text_color="#4CAF50", font=ctk.CTkFont(size=14)).grid(pady=30)
            else:
                for t in atr:
                    ...
                    ctk.CTkLabel(row, text=t['titulo'], font=ctk.CTkFont(size=13, weight="bold"),
                                 text_color="#F44336", anchor="w").grid(row=0, column=0, sticky="w", padx=16, pady=(10, 2))
                    ctk.CTkLabel(row, text=f"{t['disciplina'] or '—'}  •  Atrasada há {abs(dias_restantes(t['data']))} dias",
                                 font=ctk.CTkFont(size=11), text_color="#aa6666", anchor="w").grid(
                                     row=1, column=0, sticky="w", padx=16, pady=(0, 10))
        except Exception as e:
            ctk.CTkLabel(self.scroll_atr, text=f"Erro: {e}", text_color="#F44336").grid(pady=10)
    ...
    def _load_prioridade(self):
        ...
        try:
            ...
            for n in [4, 3, 2, 1]:
                ...
                ctk.CTkLabel(header, text=f"● {PRIORIDADE[n]}", font=ctk.CTkFont(size=13, weight="bold"),
                             text_color=cor, anchor="w").grid(padx=16, pady=10, sticky="w")
                cur.execute("SELECT * FROM tarefas WHERE prioridade=%s AND ok=FALSE", (n,))
                lista = cur.fetchall()
                if not lista:
                    ctk.CTkLabel(self.scroll_prio, text="  Nenhuma tarefa pendente",
                                 font=ctk.CTkFont(size=11), text_color="#444458").grid(sticky="w", padx=24)
                else:
                    for t in lista:
                        row = ctk.CTkFrame(self.scroll_prio, fg_color="#13131b", corner_radius=6)
                        row.grid(sticky="ew", padx=16, pady=2)
                        row.grid_columnconfigure(0, weight=1)
                        ctk.CTkLabel(row, text=f"  {t['titulo']}", font=ctk.CTkFont(size=12),
                                     text_color="#c0c0d8", anchor="w").grid(row=0, column=0, sticky="w", padx=8, pady=8)
                        ctk.CTkLabel(row, text=t['data'].strftime('%d/%m/%Y'),
                                     font=ctk.CTkFont(size=11), text_color="#555570").grid(row=0, column=1, padx=12)
            conn.close()
        except Exception as e:
            ctk.CTkLabel(self.scroll_prio, text=f"Erro: {e}", text_color="#F44336").grid(pady=10)
